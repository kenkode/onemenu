...
$tbl_footer = '</table>';
$tbl = '';
$sel=mysqli_query($con,"select * from customer left join orders on customer.cust_id=orders.customer_id left join meal on orders.meal_id=meal.m_id left join hotels on orders.hotels_id=hotels.h_id where date_ordered between '".$_REQUEST['from']."' and '".$_REQUEST['to']."' group by customer_contact") or die(mysqli_error($con));
                          $i=0;

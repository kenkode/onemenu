                          ...
                          <div class="block" style="width:890px">
                                ...
                                <div class="block-content collapse in" style="width:860px">
                                  <table class="table table-striped" style="width:860px" id="example">
                                    ...
                                    <tbody>
                                      <?php
										$sel=mysqli_query($con,"select * from orders left join meal on orders.meal_id=meal.m_id left join hotels on orders.hotels_id=hotels.h_id left join customer on orders.customer_id=customer.cust_id ") or die(mysqli_error($con));
										$i=0;
										while($row=mysqli_fetch_array($sel,MYSQLI_ASSOC)){
										$i++;
										$id=$row['order_id'];
										echo'<tr class="del'.$id.'"><td>'.$i.'</td><td><a href="view_orders.php?id='.$row['order_id'].'">'.$row['meal_name'].'</a></td><td>'.$row['food_type'].'</td><td>'.$row['hotel_name'].'</td><td>'.$row['customer_fname'].' '.$row['customer_lname'].'</td><td>'.$row['amount'].'</td><td>'.$row['date_ordered'].'</td><td><a href="view_orders.php?id='.$row['order_id'].'" class="btn btn-info">View</a></td><td><a href="reportorders.php?id='.$row['order_id'].'" target="_blank" class="btn btn-success">Report</a></td></tr>';
										}
                                            ?>
                                    </tbody>

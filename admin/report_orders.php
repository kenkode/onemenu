...
$tbl_footer = '</table>';
$tbl = '';
$sel=mysqli_query($con,"select * from orders left join meal on orders.meal_id=meal.m_id left join hotels on orders.hotels_id=hotels.h_id left join customer on orders.customer_id=customer.cust_id where date_ordered between '".$_REQUEST['from']."' and '".$_REQUEST['to']."'") or die(mysqli_error($con));
                    $i=0;
...
$profits=mysqli_query($con,"select sum(amount) as total from orders left join meal on orders.meal_id=meal.m_id left join hotels on 
         orders.hotels_id=hotels.h_id left join customer on orders.customer_id=customer.cust_id where date_ordered between '".$_REQUEST['from']."' and '".$_REQUEST['to']."'") or die(mysqli_error($con));
            
                                       $row1=mysqli_fetch_array($profits,MYSQLI_ASSOC);
                                       $total=asMoney($row1['total']);
                                       $tbl .= '<tr><td colspan="6" align="right">Total Profits</td><td align="right">'.$total.'</td></td>';  

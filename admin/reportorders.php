...
$sel=mysqli_query($con,"select * from orders left join meal on orders.meal_id=meal.m_id left join hotels on orders.hotels_id=hotels.h_id left join customer on orders.customer_id=customer.cust_id  where orders.order_id='".$_REQUEST['id']."'") or die(mysqli_error($con));
